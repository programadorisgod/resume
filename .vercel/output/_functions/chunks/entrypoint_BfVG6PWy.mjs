...
async function callMiddleware(onRequest, apiContext, responseFunction) {
  ...
  let responseFunctionPromise = undefined;
...
function validateGetStaticPathsResult(result, logger, route) {
  ...
  result.forEach((pathObject) => {
    ...
    if (pathObject.params === undefined || pathObject.params === null || pathObject.params && Object.keys(pathObject.params).length === 0) {
      throw new AstroError({
...
function generatePaginateFunction(routeMatch) {
  return function paginateUtility(data, args = {}) {
    ...
    const result = [...Array(lastPage).keys()].map((num) => {
      ...
      const next = pageNum === lastPage ? undefined : correctIndexRoute(routeMatch.generate({ ...params, page: String(pageNum + 1) }));
      const prev = pageNum === 1 ? undefined : correctIndexRoute(
        routeMatch.generate({
          ...params,
          page: !includesFirstPageNumber && pageNum - 1 === 1 ? undefined : String(pageNum - 1)
        })
      );
      const first = pageNum === 1 ? undefined : correctIndexRoute(
        routeMatch.generate({
          ...params,
          page: includesFirstPageNumber ? "1" : undefined
        })
      );
      const last = pageNum === lastPage ? undefined : correctIndexRoute(routeMatch.generate({ ...params, page: String(lastPage) }));
      return {
        params,
        props: {
          ...additionalProps,
...
class Pipeline {
  constructor(logger, manifest, mode, renderers, resolve, serverLike, streaming, adapterName = manifest.adapterName, clientDirectives = manifest.clientDirectives, inlinedScripts = manifest.inlinedScripts, compressHTML = manifest.compressHTML, i18n = manifest.i18n, middleware = manifest.middleware, routeCache = new RouteCache(logger, mode), site = manifest.site ? new URL(manifest.site) : undefined, defaultRoutes = createDefaultRoutes(manifest)) {
    this.logger = logger;
...
class RenderContext {
  constructor(pipeline, locals, middleware, pathname, request, routeData, status, cookies = new AstroCookies(request), params = getParams(routeData, pathname), url = new URL(request.url), props = {}, partial = undefined) {
  ...
  async createResult(mod) {
    ...
    const actionResult = hasActionPayload(this.locals) ? deserializeActionResult(this.locals._actionPayload.actionResult) : undefined;
    const result = {
      base: manifest.base,
      cancelled: false,
      clientDirectives,
  ...
  computeCurrentLocale() {
    ...
    const fallbackTo = strategy === "pathname-prefix-other-locales" || strategy === "domains-prefix-other-locales" ? defaultLocale : undefined;
...
array$1.splitWhen = array$1.flatten = undefined;
...
errno$1.isEnoentCodeError = undefined;
...
fs$7.createDirentFromStats = undefined;
...
path$9.convertPosixPathToPattern = path$9.convertWindowsPathToPattern = path$9.convertPathToPattern = path$9.escapePosixPath = path$9.escapeWindowsPath = path$9.escape = path$9.removeLeadingDotSegment = path$9.makeAbsolute = path$9.unixify = undefined;
const os = require$$1;
...
string$1.isEmpty = string$1.isString = undefined;
...
utils$k.string = utils$k.stream = utils$k.pattern = utils$k.path = utils$k.fs = utils$k.errno = utils$k.array = undefined;
const array = array$1;
utils$k.array = array;
const errno = errno$1;
utils$k.errno = errno;
...
out.statSync = out.stat = out.Settings = undefined;
...
common$6.joinPathSegments = undefined;
...
async$3.readdir = async$3.readdirWithFileTypes = async$3.read = undefined;
const fsStat$5 = out;
...
sync$5.readdir = sync$5.readdirWithFileTypes = sync$5.read = undefined;
const fsStat$4 = out;
...
out$1.Settings = out$1.scandirSync = out$1.scandir = undefined;
const async = async$3;
const sync$4 = sync$5;
...
common$3.joinPathSegments = common$3.replacePathSegmentSeparator = common$3.isAppliedFilter = common$3.isFatalError = undefined;
...
out$2.Settings = out$2.walkStream = out$2.walkSync = out$2.walk = undefined;
